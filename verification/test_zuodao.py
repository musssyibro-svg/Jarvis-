...
FAKE_TASK_IDS = {
    "zd_cat_data", "zd_cat_translate", "zd_cat_label",
    "zd_cat_code", "zd_cat_write",
}
...
def run() -> dict:
    t   = TestRun("test_zuodao")
    ram = [t.ram("start")]

    fetch = guard(t, "import fetch_tasks",
                  lambda: __import__("services.zuodao_service",
                                     fromlist=["fetch_tasks"]).fetch_tasks)
    if fetch is None:
        t.add_artifact(save_ram_artifact(ram, "zuodao"))
        return t.finish()

    t.log("Calling fetch_tasks(max_tasks=10)")
    ram.append(t.ram("before_fetch"))
    tasks = guard(t, "call fetch_tasks", lambda: fetch(10))
    ...
    t.check("at least one real task returned",
            ...
            error=None if tasks else "zero tasks — site needs login or unavailable")
