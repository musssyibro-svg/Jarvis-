...
export default function Agents() {
  ...
  useEffect(() => {
    const connect = () => {
      ...
      es.onmessage = e => { try { const d=JSON.parse(e.data); if(!d.ping) setFeed(f=>[...f.slice(-149),d]) } catch{} }
  ...
  useEffect(() => { if(tab==='desktop') loadDesktop(); if(tab==='memory') loadMemory(); if(tab==='planner') loadPlans() }, [tab])

  const loadStatus  = async () => { try { setCmdStatus(await apiFetch(`${API}/agents/status`)) } catch{} }
  const loadDesktop = async () => { try { setDeskStatus(await apiFetch(`${API}/agents/desktop/status`)); setVisStatus(await apiFetch(`${API}/agents/vision/status`)) } catch{} }
  const loadMemory  = async () => { try { setMemory(await apiFetch(`${API}/agents/memory/status`)) } catch{} }
  const loadPlans   = async () => { try { setPlans((await apiFetch(`${API}/agents/planner/plans`)).plans||[]) } catch{} }

  const sendCommand = async () => {
    if (!cmdInput.trim()) return
    setCmdBusy(true); setCmdResult(null)
    try { const r = await apiPost(`${API}/agents/command`, {message:cmdInput,session_id:'default'}); setCmdResult(r) }
    catch(e) { setCmdResult({response:`Error: ${e.message}`,intent:'error'}) }
  ...
  const takeScreenshot = async () => {
    try {
      ...
      setScreenshot(r)
    } catch(e) { setScreenshot({error:e.message}) }
  }

  const runOcr = async () => {
    try { const r = await apiPost(`${API}/agents/vision/ocr`, {}); setOcrResult(r) }
    catch(e) { setOcrResult({error:e.message}) }
  }

  const analyzeScreen = async () => {
    try { const r = await apiPost(`${API}/agents/vision/analyze`, {question:analyzeQ}); setOcrResult(r) }
    catch(e) { setOcrResult({error:e.message}) }
  }

  const findOnScreen = async () => {
    if (!findText.trim()) return
    try { const r = await apiPost(`${API}/agents/vision/find-text`, {text:findText}); setOcrResult(r) }
    catch(e) { setOcrResult({error:e.message}) }
  }

  const openApp = async () => {
    try { await apiPost(`${API}/agents/desktop/open-app`, {action:'open_app',app:appName}); setFeed(f=>[...f,{agent:'desktop',msg:`Opened ${appName}`,level:'success',ts:new Date().toISOString().slice(11,19)}]) }
    catch(e) { alert(e.message) }
  }

  const openFolder = async () => {
    if (!fmDir.trim()) return
    try { await apiPost(`${API}/agents/desktop/open-folder`, {path:fmDir}) }
    catch(e) { alert(e.message) }
  }

  const searchFiles = async () => {
    if (!fmDir.trim() || !fmQuery.trim()) { alert('Enter folder path and search query'); return }
    try { setFmResults(await apiPost(`${API}/agents/desktop/search-files`, {directory:fmDir,query:fmQuery})) }
    catch(e) { setFmResults({error:e.message}) }
  }

  const typeOnScreen = async () => {
    if (!typeText.trim()) return
    try { await apiPost(`${API}/agents/desktop/type`, {action:'type',text:typeText}); setFeed(f=>[...f,{agent:'desktop',msg:`Typed text`,level:'success',ts:new Date().toISOString().slice(11,19)}]) }
    catch(e) { alert(e.message) }
  }
  ...
  const createPlan = async (exec=false) => {
    ...
    try {
      ...
      setPlanResult(r); loadPlans()
    } catch(e) { setPlanResult({error:e.message}) }
  }

  const executePlan = async (id) => {
    try { await apiPost(`${API}/agents/planner/${id}/execute`, {}); loadPlans() } catch(e) { alert(e.message) }
  }
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
          <div>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:6 }}>
                  {[
                    ...
                    <div key={name} style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                      <span style={{ fontFamily:'monospace',fontSize:10,color:'rgba(255,255,255,0.5)' }}><Dot on={ok}/>{name}Agent</span>
                      <span style={{ fontFamily:'monospace',fontSize:9,color:ok?C.green:'rgba(255,255,255,0.2)' }}>{ok?'ONLINE':'OFFLINE'}</span>
                    </div>
                  ))}
                </div>
            ...
            <div style={card}>
              ...
              <p style={{ fontSize:11,color:'rgba(255,255,255,0.35)',lineHeight:1.6,marginBottom:12 }}>
          ...
          <div>
            {cmdResult && (
              <div style={card}>
                <div style={{ display:'flex',justifyContent:'space-between',marginBottom:8 }}>
                  ...
                  <span style={{ fontFamily:'monospace',fontSize:8,padding:'2px 7px',borderRadius:2,border:`1px solid ${C.accent}44`,color:C.accent }}>{cmdResult.intent?.toUpperCase()}</span>
                </div>
                <div style={{ background:'rgba(0,4,8,0.9)',padding:12,borderRadius:3,fontSize:11,color:'rgba(200,230,240,0.8)',lineHeight:1.7,whiteSpace:'pre-wrap',maxHeight:250,overflowY:'auto' }}>
                  {cmdResult.response || JSON.stringify(cmdResult, null, 2)}
                </div>
                {cmdResult.plan && (
                  <div style={{ marginTop:10 }}>
                    <div style={{ fontSize:9,color:C.orange,marginBottom:6 }}>PLAN ({cmdResult.plan.steps?.length} steps)</div>
                    {cmdResult.plan.steps?.map(s => (
                      <div key={s.step} style={{ fontSize:10,color:'rgba(255,255,255,0.4)',padding:'3px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                        <span style={{ color:C.accent,marginRight:8 }}>{s.step}.</span>{s.description}
                      </div>
                    ))}
                    {cmdResult.plan_id && (
                      <button onClick={()=>executePlan(cmdResult.plan_id)} style={{ ...btn(C.green),marginTop:10 }}>▶ EXECUTE PLAN</button>
                    )}
                  </div>
                )}
              </div>
            ...
            <div style={card}>
              ...
              <div ref={feedRef} style={{ background:'rgba(0,4,8,0.9)',borderRadius:3,padding:10,height:200,overflowY:'auto',fontFamily:'monospace',fontSize:9 }}>
                {feed.length===0&&<span style={{color:'rgba(255,255,255,0.15)'}}>Waiting…</span>}
                {feed.map((e,i)=>(
                  <div key={i} style={{lineHeight:1.9,color:LEVEL_COLOR[e.level]||C.accent}}>
                    ...
                    <span style={{color:'rgba(255,255,255,0.3)',marginRight:6}}>[{e.agent}]</span>{e.msg}
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
          <div>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:5 }}>
                  {Object.entries(deskStatus.capabilities||{}).map(([k,v])=>(
                    <div key={k} style={{ display:'flex',justifyContent:'space-between',fontSize:10,padding:'4px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                      ...
                      <span style={{ color:v?C.green:C.red,fontFamily:'monospace' }}>{v?'✓':'✗'}</span>
                    </div>
                  ))}
                  {deskStatus.screen && <div style={{ fontSize:9,color:'rgba(255,255,255,0.25)',fontFamily:'monospace',marginTop:8 }}>Screen: {deskStatus.screen.width}×{deskStatus.screen.height} | Mouse: {deskStatus.screen.mouse_x},{deskStatus.screen.mouse_y}</div>}
                </div>
              ) : <button onClick={loadDesktop} style={btn()}>Load Status</button>}
              {!deskStatus?.pyautogui && (
                <div style={{ marginTop:10,padding:'8px 10px',background:'rgba(255,149,0,0.08)',border:'1px solid rgba(255,149,0,0.25)',borderRadius:3,fontSize:10,color:C.orange }}>
            ...
            <div style={card}>
              ...
              <div style={{ display:'flex',gap:7,marginBottom:8 }}>
                <input value={appName} onChange={e=>setAppName(e.target.value)} style={{...inp,flex:1}} placeholder="notepad, chrome, edge…" />
                <button onClick={openApp} style={btn(C.green)}>OPEN</button>
              </div>
            ...
            <div style={card}>
              ...
              <textarea value={typeText} onChange={e=>setTypeText(e.target.value)} rows={3} style={{...inp,marginBottom:8}} placeholder="Text to type on screen…" />
              <button onClick={typeOnScreen} style={btn(C.green)}>TYPE ON SCREEN</button>
            </div>
          </div>
          ...
          <div>
            <div style={card}>
              ...
              {[
                ['Take Screenshot', ()=>apiPost(`${API}/agents/desktop/screenshot`,{}).then(r=>setScreenshot(r)),'#00d4ff'],
                ['Copy (Ctrl+C)',   ()=>apiPost(`${API}/agents/execute`,{action:'hotkey',params:{keys:['ctrl','c']}}),'#00ff88'],
                ['Paste (Ctrl+V)',  ()=>apiPost(`${API}/agents/execute`,{action:'hotkey',params:{keys:['ctrl','v']}}),'#00ff88'],
                ['Select All',     ()=>apiPost(`${API}/agents/execute`,{action:'hotkey',params:{keys:['ctrl','a']}}),'#ff9500'],
                ['Press Escape',   ()=>apiPost(`${API}/agents/execute`,{action:'press',params:{key:'escape'}}),'#ff4444'],
                ['Press Enter',    ()=>apiPost(`${API}/agents/execute`,{action:'press',params:{key:'enter'}}),'#a78bfa'],
              ].map(([label,fn,c])=>(
                <button key={label} onClick={fn} style={{ ...btn(c),marginBottom:6,marginRight:6,display:'block',width:'100%',textAlign:'left' }}>{label}</button>
              ))}
            </div>
            ...
            <div style={card}>
              ...
              <div style={{ display:'flex',gap:7 }}>
                <input id="closeApp" style={{...inp,flex:1}} placeholder="chrome, notepad, discord…" />
                <button onClick={()=>{const v=document.getElementById('closeApp').value; if(v) apiPost(`${API}/agents/desktop/close-app`,{app:v})}} style={btn(C.red)}>CLOSE</button>
              </div>
            </div>
            ...
            <div style={card}>
              ...
              <div style={{ display:'flex',gap:7,marginBottom:8 }}>
                <input value={fmDir} onChange={e=>setFmDir(e.target.value)} style={{...inp,flex:1}} placeholder="Folder path (e.g. C:\\Users\\You\\Documents)" />
                <button onClick={openFolder} style={btn(C.accent)}>OPEN</button>
              </div>
              <div style={{ display:'flex',gap:7,marginBottom:8 }}>
                <input value={fmQuery} onChange={e=>setFmQuery(e.target.value)} style={{...inp,flex:1}} placeholder="Search filename…" />
                <button onClick={searchFiles} style={btn(C.green)}>SEARCH</button>
              </div>
              {fmResults && (
                <div style={{ background:'rgba(0,4,8,0.8)',borderRadius:3,padding:10,maxHeight:200,overflowY:'auto',marginTop:6 }}>
                  {fmResults.error && <div style={{color:C.red,fontSize:11}}>{fmResults.error}</div>}
                  {fmResults.matches?.length===0 && <div style={{color:'rgba(255,255,255,0.3)',fontSize:11}}>No matches</div>}
                  {fmResults.matches?.map((m,i)=>(
                    <div key={i} style={{ display:'flex',justifyContent:'space-between',padding:'4px 0',borderBottom:'1px solid rgba(255,255,255,0.04)',fontSize:10 }}>
                      <span style={{color:m.is_dir?C.orange:'rgba(255,255,255,0.6)',fontFamily:'monospace'}}>{m.is_dir?'📁':'📄'} {m.name}</span>
                      <button onClick={()=>{if(confirm(`Delete ${m.name}?`)) apiPost(`${API}/agents/desktop/delete-file`,{path:m.path,confirm:true}).then(()=>searchFiles())}} style={{ background:'none',border:'none',color:C.red,cursor:'pointer',fontSize:9 }}>✕</button>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
          <div>
            ...
            <div style={card}>
              ...
              <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
                <button onClick={takeScreenshot} style={btn(C.accent)}>📷 TAKE SCREENSHOT</button>
                <button onClick={runOcr} style={btn(C.green)}>🔍 READ TEXT (OCR)</button>
                <div style={{ height:1,background:'rgba(255,255,255,0.06)',margin:'2px 0' }} />
                <input value={analyzeQ} onChange={e=>setAnalyzeQ(e.target.value)} style={inp} placeholder="Ask about the screen…" />
          ...
          <div>
            {screenshot && (
              <div style={card}>
                ...
                {screenshot.error && <div style={{ color:C.red,fontSize:11 }}>{screenshot.error}</div>}
              </div>
            )}
            {ocrResult && (
              <div style={card}>
                ...
                {ocrResult.found !== undefined && (
                  <div style={{ fontFamily:'monospace',fontSize:11,color:ocrResult.found?C.green:C.red,marginBottom:8 }}>
                ...
                {ocrResult.text && (
                  <div style={{ background:'rgba(0,4,8,0.8)',padding:10,borderRadius:3,fontFamily:'monospace',fontSize:9,color:'rgba(200,230,240,0.5)',maxHeight:200,overflowY:'auto',whiteSpace:'pre-wrap' }}>
                    {ocrResult.text.slice(0,1500)}
                  </div>
                )}
                {ocrResult.error && <div style={{ color:C.red,fontSize:11 }}>{ocrResult.error}</div>}
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
          <div>
            <div style={card}>
              ...
              <textarea value={planGoal} onChange={e=>setPlanGoal(e.target.value)} rows={3} style={{...inp,marginBottom:10}} placeholder="Goal: Apply for 3 Python automation jobs today&#10;Goal: Check my Freelancer messages and draft replies&#10;Goal: Open Chrome and search for remote Python jobs" />
              <div style={{ display:'flex',gap:7 }}>
                <button onClick={()=>createPlan(false)} style={btn(C.accent)}>CREATE PLAN</button>
                <button onClick={()=>createPlan(true)} style={btn(C.green)}>CREATE + EXECUTE</button>
              </div>
            ...
            {planResult && (
              <div style={card}>
                ...
                {planResult.error && <div style={{ color:C.red,fontSize:11 }}>{planResult.error}</div>}
                {planResult.plan && (
                  <>
                    <div style={{ fontSize:11,color:'rgba(255,255,255,0.5)',marginBottom:10 }}>{planResult.plan.goal}</div>
                    {planResult.plan.steps?.map(s=>(
                      <div key={s.step} style={{ display:'flex',gap:10,padding:'6px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                        <span style={{ fontFamily:'monospace',fontSize:10,color:C.accent,minWidth:20 }}>{s.step}.</span>
                        <div>
                          <div style={{ fontSize:10,color:'rgba(255,255,255,0.6)' }}>{s.description}</div>
                          <div style={{ fontSize:9,color:'rgba(255,255,255,0.25)',fontFamily:'monospace' }}>{s.action}({JSON.stringify(s.params)})</div>
                        </div>
                      </div>
                    ))}
                    {planResult.plan_id && !planResult.executing && (
                      <button onClick={()=>executePlan(planResult.plan_id)} style={{ ...btn(C.green),marginTop:10 }}>▶ EXECUTE THIS PLAN</button>
                    )}
                    {planResult.executing && <div style={{ fontSize:10,color:C.green,fontFamily:'monospace',marginTop:8 }}>● Executing in background — watch the feed</div>}
                  </>
                )}
              </div>
            )}
          ...
          <div>
            <div style={card}>
              ...
              {plans.map(p=>(
                <div key={p.id} style={{ padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display:'flex',justifyContent:'space-between',marginBottom:4 }}>
                    <span style={{ fontSize:11,color:'rgba(255,255,255,0.6)' }}>{p.goal?.slice(0,50)}</span>
                    <span style={{ fontFamily:'monospace',fontSize:8,color:p.status==='done'?C.green:p.status==='running'?C.orange:C.accent }}>
                      {p.status?.toUpperCase()}
                    </span>
                  </div>
                  <div style={{ display:'flex',gap:6 }}>
                    <button onClick={()=>executePlan(p.id)} style={{ ...btn(C.green),fontSize:8,padding:'3px 10px' }}>▶ RUN</button>
                    <span style={{ fontSize:9,color:'rgba(255,255,255,0.2)',fontFamily:'monospace',alignSelf:'center' }}>{p.created_at?.slice(0,16)}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
          <div>
            <div style={card}>
              ...
              {memory ? (
                <div style={{ display:'flex',flexDirection:'column',gap:5 }}>
                  {[['Win Patterns',  memory.win_patterns],['Fail Patterns', memory.fail_patterns],['KV Records',     memory.kv_count]].map(([k,v])=>(
                    <div key={k} style={{ display:'flex',justifyContent:'space-between',fontSize:11,padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                      <span style={{ color:'rgba(255,255,255,0.4)' }}>{k}</span>
                      <span style={{ fontFamily:'monospace',color:C.accent }}>{v}</span>
                    </div>
                  ))}
                  {memory.platforms?.map(p=>(
                    <div key={p.platform} style={{ display:'flex',justifyContent:'space-between',fontSize:11,padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                      <span style={{ color:'rgba(255,255,255,0.4)' }}>{p.platform}</span>
                      <span style={{ fontFamily:'monospace',color:p.win_rate>30?C.green:C.orange }}>{p.win_rate}% win ({p.total_won}/{p.total_sent})</span>
                    </div>
                  ))}
                </div>
              ) : <button onClick={loadMemory} style={btn()}>Load Memory</button>}
            </div>

            <div style={card}>
              <div style={label}>AI INSIGHTS</div>
              <button onClick={loadInsights} style={{ ...btn(C.purple),marginBottom:12 }}>🧠 GENERATE INSIGHTS</button>
              {insights && (
                <>
                  {insights.error && <div style={{ color:C.red,fontSize:11 }}>{insights.error}</div>}
                  {insights.insights?.map((ins,i)=>(
                    <div key={i} style={{ display:'flex',gap:8,padding:'6px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                      <span style={{ color:C.purple,fontFamily:'monospace',fontSize:10 }}>→</span>
                      <span style={{ fontSize:11,color:'rgba(200,230,240,0.7)',lineHeight:1.6 }}>{ins}</span>
                    </div>
                  ))}
                </>
              )}
          ...
          <div>
            <div style={card}>
              ...
              <OutcomeForm onSave={async(data)=>{ await apiPost(`${API}/agents/memory/outcome`,data); loadMemory() }} />
            </div>
          </div>
        </div>
      )}
      ...
      {tab==='feed' && (
        <div style={card}>
          ...
          <div style={{ background:'rgba(0,4,8,0.95)',borderRadius:3,padding:14,height:'calc(100vh - 220px)',overflowY:'auto',fontFamily:'monospace',fontSize:10 }}>
            ...
            {feed.map((e,i)=>(
              <div key={i} style={{ lineHeight:2,borderBottom:'1px solid rgba(255,255,255,0.03)' }}>
                <span style={{color:'rgba(255,255,255,0.18)',marginRight:8,fontSize:9}}>{e.ts}</span>
                <span style={{marginRight:8,padding:'1px 6px',borderRadius:2,fontSize:8,background:`${LEVEL_COLOR[e.level]||C.accent}18`,color:LEVEL_COLOR[e.level]||C.accent,border:`1px solid ${LEVEL_COLOR[e.level]||C.accent}33`}}>{e.agent}</span>
...
function OutcomeForm({ onSave }) {
  ...
  const submit = async () => {
    await onSave({ platform, job_type:jobType, proposal_snippet:snippet, won, client_response:response })
    setSnippet(''); setResponse('')
  }
  const inp = { width:'100%',background:'rgba(0,10,20,0.9)',border:'1px solid rgba(0,212,255,0.2)',outline:'none',color:'#c8e8f0',padding:'7px 10px',fontSize:11,fontFamily:'monospace',borderRadius:3,boxSizing:'border-box',marginBottom:8 }
  const sel = {...inp,cursor:'pointer'}
  return (
    <div>
      <select value={platform} onChange={e=>setPlatform(e.target.value)} style={sel}>
        {['freelancer','hubstaff','remoteok','weworkremotely','peopleperhour','contra'].map(p=><option key={p} value={p}>{p}</option>)}
      </select>
      <select value={jobType} onChange={e=>setJobType(e.target.value)} style={sel}>
        {['code','writing','research','data','email','seo','automation','other'].map(t=><option key={t} value={t}>{t}</option>)}
      </select>
      <textarea value={snippet} onChange={e=>setSnippet(e.target.value)} rows={3} style={inp} placeholder="Proposal snippet (first 100 words)…" />
      <textarea value={response} onChange={e=>setResponse(e.target.value)} rows={2} style={inp} placeholder="Client response (optional)…" />
      <div style={{ display:'flex',gap:10,alignItems:'center',marginBottom:10 }}>
        <label style={{ fontSize:11,color:'rgba(255,255,255,0.5)',display:'flex',alignItems:'center',gap:6,cursor:'pointer' }}>
          <input type="checkbox" checked={won} onChange={e=>setWon(e.target.checked)} style={{ accentColor:'#00ff88' }} />
          Won this project
        </label>
      </div>
      <button onClick={submit} style={{ padding:'7px 16px',borderRadius:3,border:'1px solid #00ff88',background:'rgba(0,255,136,0.1)',color:'#00ff88',fontFamily:'monospace',fontSize:9,cursor:'pointer' }}>
        💾 SAVE OUTCOME
      </button>
    </div>
  )
