...
export default function Settings() {
  ...
  const load = useCallback(async () => {
    const grab = async (path, set, pick = (x) => x) => {
      try {
        set(pick(await fetch(`${API}${path}`).then((r) => r.json())));
      } catch {}
    };
    grab(
      "/settings",
    ...
    grab("/settings/effective", setEffective, (j) =>
      Object.entries(j.settings || {}).map(([key, v]) => ({ key, ...v }))
    );
  ...
  const rescan = async () => {
    try {
      setEnv(await fetch(`${API}/os/environment?refresh=true`).then((r) => r.json()));
    } catch {}
  };
    ...
    <div style={{ padding: 22, maxWidth: 860 }}>
      ...
      <section style={card}>
        ...
        <div style={{ fontSize: 12, color: T.dim, marginBottom: 14, lineHeight: 1.5 }}>
          What Jarvis remembers so it stops asking. Anything you set here beats what it detected,
          and a machine scan will never overwrite it.
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
          ...
          >
            ...
            {(persona?.unknown || []).map((u) => (
              <option key={u.field} value={u.field}>
                {u.field.replace(/_/g, " ")}
              </option>
            ))}
            {(persona?.known || []).map((u) => (
          ...
          <input
            ...
            style={input}
          />
          <button style={btn(T.green)} onClick={addFact}>
            Add
          </button>
        </div>
        {newFact.field && (
      ...
      <section style={card}>
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <button style={btn(T.cyan)} onClick={rescan}>
            Scan again
          </button>
        </div>
        ...
        {env && (
          ...
          >
            ...
            <Fact
              k="Messaging"
              v={
                ["qq", "wechat", "telegram", "dingtalk", "discord", "slack"]
                  .filter((a) => env.apps?.[a])
                  .join(", ") || "none found"
              }
            />
            ...
            <Fact
              k="Models"
              v={`${env.ollama?.models?.length || 0} pulled · ${env.ollama?.total_gb || 0} GB`}
            />
          </div>
        )}
        ...
        {(env?.constraints || []).map((c) => (
          ...
          >
            ...
            {c.fix && (
              <div style={{ fontSize: 11.5, color: T.green, marginTop: 3 }}>Fix: {c.fix}</div>
            )}
          </div>
        ))}
      </section>
      ...
      <section style={card}>
        ...
        <Field
          l="Reasoning model"
          value={cfg.ollama_reasoning_model}
        ...
        <Field
          l="Fast model"
          value={cfg.ollama_fast_model}
          onChange={set("ollama_fast_model")}
        ...
        <Field
          ...
          onChange={set("ollama_vision_model")}
        ...
        <div style={{ fontSize: 12, color: T.dim, marginBottom: 12, lineHeight: 1.5 }}>
          Leave blank and Jarvis picks whatever is installed and has been working. Set one to force
          it.
        </div>
        ...
        <button style={{ ...btn(T.green), marginTop: 8 }} onClick={save}>
          Save
        </button>
      ...
      <section style={card}>
        ...
        <div style={{ fontSize: 12, color: T.dim, marginBottom: 14, lineHeight: 1.5 }}>
          Everything runs on your own PC unless you add a key below. Nothing is sent anywhere by
          default. If a paid one fails or runs out of credit, Jarvis falls back to the local model
          rather than stopping.
        </div>

        {ai &&
          Object.entries(ai.routing || {}).map(([task, provider]) => (
            <div
              key={task}
        ...
        <div style={{ fontSize: 12, color: T.dim, marginBottom: 12, lineHeight: 1.5 }}>
          Paste a key and that provider becomes selectable above. Keys are stored on this PC and are
          never shown in reports — only the first and last few characters are ever displayed back.
        </div>
        {["deepseek", "glm", "kimi", "openrouter", "gemini", "openai"].map((p) => {
          ...
          return (
            <div key={p} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
              <div style={{ width: 96, fontSize: 11.5, color: info.configured ? T.green : T.dim }}>
                {info.label || p}
              </div>
              <input
                type="password"
                placeholder="API key (leave blank to keep off)"
                value={cfg[`ai_${p}_api_key`] || ""}
                style={{ ...input, flex: 1 }}
                onChange={(e) => set(`ai_${p}_api_key`)(e.target.value)}
              />
              <input
                placeholder="model (optional)"
                value={cfg[`ai_${p}_model`] || ""}
        ...
        {ai?.last_used && Object.keys(ai.last_used).length > 0 && (
          <div style={{ marginTop: 14, fontSize: 11.5, color: T.dim }}>
            ...
            {Object.entries(ai.last_used)
              .map(([t, u]) => `${t}: ${u.provider}`)
              .join(" · ")}
          </div>
        )}

        <button style={{ ...btn(T.green), marginTop: 12 }} onClick={save}>
          Save
        </button>
      </section>
      ...
      <section style={card}>
        ...
        <div style={{ fontSize: 11.5, color: T.dim, marginTop: 14, lineHeight: 1.6 }}>
          Jarvis can type on your keyboard and use a browser already signed in to your accounts.
          Keep it on 127.0.0.1. Set DESKTOP_CONTROL_ENABLED=0 in backend/.env to let the freelance
          engine keep running while nothing touches your desktop.
        </div>
      </section>
      ...
      <section style={card}>
        ...
        <div style={{ fontSize: 12, color: T.dim, marginBottom: 14, lineHeight: 1.5 }}>
          If you changed something and nothing happened, this is where you find out why — the source
          column shows whether the running system is using your value or overriding it.
        </div>
        <div
          style={{
            display: "grid",
...
function Fact({ k, v }) {
  return (
    <div>
      <div
        style={{ fontSize: 10, color: T.dim, letterSpacing: "0.1em", textTransform: "uppercase" }}
      >
        {k}
      </div>
    ...
    <>
      ...
      <div style={{ color: T.text, wordBreak: "break-all" }}>
        {s.value === "" || s.value == null ? (
          <span style={{ color: T.dim }}>(empty)</span>
        ) : (
          String(s.value)
        )}
      </div>
