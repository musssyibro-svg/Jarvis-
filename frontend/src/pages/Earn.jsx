...
export default function Earn() {
  ...
  const load = async () => {
    try {
      const [i, q, s, c, p, o] = await Promise.all([
        fetch(`${API}/automation/income/status`)
          .then((r) => r.json())
          .catch(() => null),
        fetch(`${API}/automation/queue?limit=100`)
          .then((r) => r.json())
          .catch(() => ({ queue: [] })),
        fetch(`${API}/sessions/status`)
          .then((r) => r.json())
          .catch(() => ({ platforms: [] })),
        fetch(`${API}/sessions/custom`)
          .then((r) => r.json())
          .catch(() => ({ platforms: [] })),
        fetch(`${API}/automation/profile`)
          .then((r) => r.json())
          .catch(() => null),
        fetch(`${API}/automation/queue/outcome`)
          .then((r) => r.json())
          .catch(() => null),
      ]);
      ...
      setIncome(i);
      setQueue(q.queue || []);
      setSessions(s.platforms || []);
      setCustom(c.platforms || []);
      setProfile(p);
      setOutcome(o);
    } catch {
      /* backend down */
  ...
  const runNow = async () => {
    await fetch(`${API}/automation/income/run-now`, { method: "POST" });
    setMsg("Running a cycle now…");
    load();
  };
  ...
  const addPlatform = async () => {
    if (!form.label || !form.jobs_url) {
      setMsg("Name and jobs URL are required.");
      return;
    }
    ...
    setMsg(
      r.ok
        ? `Added ${d.label}${d.credentials_saved ? " (login saved, encrypted)" : ""}. It'll be scanned from the next cycle.`
        : d.detail || "Couldn't add that site."
    );
    if (r.ok) {
      setForm({
        label: "",
  ...
  const submitApproved = () =>
    act("/automation/execute-approved", "Submitting approved proposals…");
  ...
  const submitted = count("done"),
    ready = count("ready"),
    pending = count("pending"),
    needLogin = count("needs_login");

  return (
    <div style={{ padding: "20px 22px", display: "flex", flexDirection: "column", gap: 18 }}>
      {/* THE switch */}
      <div
        style={{ ...card, borderColor: on ? "rgba(62,230,168,0.4)" : T.line }}
        className={on ? "alive" : ""}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: on ? T.green : T.text }}>
              {confirming
                ? on
                  ...
                  : "Stopping…"
                : on
                  ? "Jarvis is earning for you"
                  : "Autonomous earning is off"}
            </div>
          ...
          <button
            ...
            style={{
              ...
              background: on ? "rgba(255,95,109,0.15)" : T.green,
              color: on ? T.red : "#052018",
            }}
          >
        ...
        >
          {[
            ...
            ["Drafted", pending, T.violet],
          ].map(([k, v, c]) => (
            <div
              key={k}
              style={{ background: "rgba(0,0,0,0.25)", borderRadius: 10, padding: "10px 12px" }}
            >
              <div
                style={{
                  fontSize: 9,
              ...
              <div className="smooth" style={{ fontSize: 20, fontWeight: 700, color: c }}>
                {v}
              </div>
            </div>
          ))}
        ...
        {(pending > 0 || queue.some((q) => q.status === "approved")) && (
          ...
          >
            ...
            <span style={{ fontSize: 12, color: T.dim }}>
              Approving sends nothing. Submit does — and saves a receipt you can open with "Proof".
            </span>
          </div>
        )}
                ...
                {outcome.recent.slice(0, 4).map((r) => (
                  ...
                  >
                    <span
                      style={{
                        color:
                          r.status === "done" ? T.green : r.status === "failed" ? T.red : T.amber,
                      }}
                    >
                      {r.status === "done"
                        ? r.confirmed_by_site === false
                          ? "sent, unconfirmed"
                          : "sent"
        ...
        <label
          style={{
            ...
            marginTop: 14,
            fontSize: 12.5,
            color: profile?.auto_submit ? T.amber : T.dim,
            cursor: "pointer",
          }}
        >
          <input
            type="checkbox"
            checked={!!profile?.auto_submit}
      ...
      {msg && (
        <div
          style={{ ...card, borderColor: "rgba(84,214,255,0.3)", fontSize: 12.5, color: T.cyan }}
        >
          {msg}
        </div>
      )}
      ...
      {receipt && (
        <div style={{ ...card, borderColor: "rgba(84,214,255,0.35)" }}>
          <div style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
            <div style={{ ...lbl, flex: 1, marginBottom: 0 }}>What Jarvis actually sent</div>
            <button
                ...
                <div style={{ fontSize: 12.5, color: T.dim, lineHeight: 1.8 }}>
                  ...
                  {receipt.receipt.final_url && (
                    <div style={{ wordBreak: "break-all" }}>
                      Ended on: {receipt.receipt.final_url}
                    </div>
                  )}
                </div>
              )}
              ...
              {receipt.proof_url && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <a href={`${API}${receipt.proof_url}`} target="_blank" rel="noreferrer">
                    <img
                      src={`${API}${receipt.proof_url}`}
                      alt="submission proof"
                      style={{ maxWidth: "100%", borderRadius: 9, border: `1px solid ${T.line}` }}
                    />
                  </a>
                </div>
              )}
      ...
      {(needLogin > 0 || sessions.some((s) => s.logged_in === false)) && (
        <div style={{ ...card, borderColor: "rgba(245,181,68,0.35)" }}>
          ...
          <div style={{ fontSize: 12.5, color: T.dim, marginBottom: 10 }}>
            Jarvis can only submit on sites you're logged into. Log in once — the session is reused
            forever.
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {sessions
              .filter((s) => s.logged_in !== true)
              .map((s) => (
                <button
                  key={s.platform}
                  onClick={() => openLogin(s.platform)}
                  style={{
                    padding: "7px 13px",
                    borderRadius: 9,
                    fontSize: 12,
        ...
        <div style={{ ...card, padding: 0, overflow: "hidden" }}>
          {queue.length === 0 && (
            <div style={{ padding: 16, color: T.dim, fontSize: 13 }}>
              Nothing yet.{" "}
              {on
                ? "The first cycle will appear here shortly."
                : "Turn on autonomous earning above."}
            </div>
          )}
          {queue.slice(0, 25).map((q) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ fontSize: 11, color: c, minWidth: 130, textAlign: "right" }}>
                  {what}
                </span>
                {link && (
                  <a
                ...
                {(q.status === "pending" || q.status === "ready") && (
                  <>
                    <button
                      onClick={() => act(`/automation/queue/${q.id}/approve`, "Approved.")}
                      style={{
                        fontSize: 11,
                        padding: "2px 9px",
                        borderRadius: 7,
                        ...
                        border: `1px solid rgba(62,230,168,0.4)`,
                        color: T.green,
                      }}
                    >
                      Approve
                    ...
                    <button
                      onClick={() => act(`/automation/queue/${q.id}/reject`, "Rejected.")}
                      style={{
                        fontSize: 11,
                        padding: "2px 9px",
                        borderRadius: 7,
        ...
        {showAdd && (
          <div style={{ ...card, marginBottom: 10 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <input
                style={field}
                placeholder="Site name (e.g. Toptal)"
                value={form.label}
              ...
              <input
                ...
                placeholder="Jobs page URL (where listings are)"
                value={form.jobs_url}
                onChange={(e) => setForm({ ...form, jobs_url: e.target.value })}
              />
              <input
                style={{ ...field, gridColumn: "1 / -1" }}
            ...
            >
              <span
                style={{
                  ...
                  borderRadius: "50%",
                  background:
                    s.logged_in === true ? T.green : s.logged_in === false ? T.red : T.dim,
                }}
              />
          ...
          {custom.map((p) => (
            <div
              ...
              style={{
                ...
                alignItems: "center",
                gap: 8,
                borderColor: "rgba(169,139,255,0.3)",
              }}
            >
              ...
              <span
                onClick={() =>
                  fetch(`${API}/sessions/custom/${p.slug}`, { method: "DELETE" }).then(load)
                }
                style={{ cursor: "pointer", color: T.red, fontSize: 13 }}
              >
                ✕
              </span>
            </div>
