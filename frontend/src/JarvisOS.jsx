...
import { card, sectionLabel, T } from "./theme.js";
...
export default function JarvisOS() {
  ...
  const freeRam = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          setRam(await fetch(`${API}/os/memory-pressure`).then((x) => x.json()));
        } catch {
          /* the poll will catch up */
        }
      }, 1500);
    } catch {
      /* nothing to do but leave the banner as it was */
    }
    setFreeing(false);
  }, []);
  ...
  const ctl = useCallback(async (cmd) => {
    setCtrl((c) => ({
      ...(c || {}),
      can_pause: cmd === "resume",
      can_resume: cmd === "pause",
      mode: cmd === "pause" ? "pausing" : cmd === "cancel" ? "cancelling" : "running",
    }));
    try {
      ...
      /* the poll below will correct it */
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (r.ok && alive) {
          ...
          setHistory((h) =>
            [...h, { cpu: d.system?.cpu ?? 0, ram: d.system?.ram ?? 0, t: Date.now() }].slice(-30)
          );
          ...
          fetch(`${API}/os/control`)
            .then((x) => x.json())
            .then(setCtrl)
            .catch(() => {});
          fetch(`${API}/os/memory-pressure`)
            .then((x) => x.json())
            .then(setRam)
            .catch(() => {});
          setFeed((f) =>
            f.length
              ...
              : (d.timeline || []).map((e) => ({
                  ...e,
                  _t: eventTime(e),
                  _k: `seed_${e.at || e.ts}_${Math.random()}`,
                }))
          );
        }
      } catch (e) {
        ...
        if (alive)
          setLink({
            ...
            fix:
              `Nothing answered at ${API}. Check the "Jarvis Backend" window is ` +
    ...
    let t = null;
    const delay = () => (document.hidden ? 30000 : osRef.current?.status?.running ? 2500 : 10000);
    const tick = () => {
      load();
    ...
    const connect = () => {
      ...
      es.onmessage = (e) => {
        try {
          ...
          setFeed((f) => [{ ...d, _t: d.at || Date.now(), _k: Math.random() }, ...f].slice(0, 120));
        } catch {
  ...
  const run = useCallback(
    async (text) => {
      ...
      setReply(null);
      setFeed((f) => [
        { ts: hhmm(), agent: "you", msg, level: "info", _t: Date.now(), _k: Math.random() },
        ...f,
      ]);
      try {
  ...
  const LINK_LABEL = {
    ...
    erroring: "Backend error",
    refused: "Refused",
    unreachable: "Offline",
  };
  const statusLabel = busy
    ? "Thinking"
    : os?.status?.label || (online ? "Ready" : LINK_LABEL[link?.state] || "Connecting…");
      ...
      */}
      {!online && link && (
        ...
        >
          ...
          {link.fix && <div style={{ color: T.dim, marginTop: 4 }}>{link.fix}</div>}
        </div>
      )}
      ...
      {ram && ram.level && ram.level !== "ok" && (
        <div
          style={{
            ...
            fontSize: 12.5,
            background:
              ram.level === "critical" ? "rgba(255,95,109,0.10)" : "rgba(245,181,68,0.09)",
            borderBottom: `1px solid ${
              ram.level === "critical" ? "rgba(255,95,109,0.3)" : "rgba(245,181,68,0.3)"
            }`,
          }}
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <span style={{ color: ram.level === "critical" ? T.red : T.amber, fontWeight: 600 }}>
              {ram.headline}
            </span>
            <div style={{ flex: 1 }} />
            <button
          ...
          {(ram.effects || []).map((e, i) => (
            <div key={i} style={{ color: T.text, marginTop: 4 }}>
              · {e}
            </div>
          ))}
          {(ram.advice || []).map((a, i) => (
            <div key={i} style={{ color: T.dim, marginTop: 4 }}>
              {a}
            </div>
          ))}
          {ram.justFreed && (
      ...
      */}
      {liveNow && (
        ...
        >
          ...
          <button
            onClick={() => ctl(ctrl?.can_resume ? "resume" : "pause")}
            style={{
              padding: "3px 10px",
              borderRadius: 7,
              fontSize: 11,
              ...
              color: ctrl?.can_resume ? T.green : T.dim,
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {why.goal && (
                <div style={{ marginBottom: 4 }}>
                  <span style={{ color: T.dim }}>Goal: </span>
                  {why.goal}
                </div>
              )}
              {(why.because || []).map((b, i) => (
                <div key={i} style={{ color: T.dim, lineHeight: 1.6 }}>
                  · {b}
                </div>
              ))}
            </div>
            ...
            {feed.map((f) => (
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div
                    style={{ fontSize: 12, color: T.text, opacity: 0.9, wordBreak: "break-word" }}
                  >
      ...
      >
        <span style={{ color: working ? T.amber : T.cyan, fontSize: 15 }}>
          {working ? "◐" : "›"}
        </span>
        <input
          value={input}
...
function VitalsGraph({ history, events, busy }) {
  ...
  const line = (key) =>
    pts.map((p, i) => `${i === 0 ? "M" : "L"} ${pad + i * step} ${y(p[key])}`).join(" ");
        ...
        <div style={{ ...card, padding: 18 }} className={os?.status?.running ? "alive" : ""}>
          {os?.goal ? (
            ...
            <div style={{ color: T.dim, fontSize: 13 }}>
              No active goal. Give Jarvis one below — or start the income engine in Earn and it'll
              work on its own.
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div
            style={{
              ...card,
              padding: 16,
              borderLeft: `2px solid ${reply?.intent === "error" ? T.red : T.cyan}`,
            }}
          >
            {busy && !reply ? (
              <span style={{ color: T.dim, fontSize: 13 }}>
                <span
                  className="think-ring"
                  style={{ color: T.amber, marginRight: 8, verticalAlign: "middle" }}
                />
                thinking…
              </span>
            ) : (
...
function Diagnostics() {
  ...
  const load = () => {
    fetch(`${API}/os/diagnostics`)
      .then((r) => r.json())
      .then(setData)
      .catch(() => setData(null));
    fetch(`${API}/os/health-check`)
      .then((r) => r.json())
      .then(setHealth)
      .catch(() => setHealth(null));
    fetch(`${API}/os/experience`)
      .then((r) => r.json())
      .then(setExp)
      .catch(() => setExp(null));
  };
      ...
      <div>
        ...
        >
          ...
          {(health?.fixes || []).length > 0 && (
            <div
              style={{ marginTop: 12, background: "rgba(0,0,0,0.3)", borderRadius: 9, padding: 12 }}
            >
              <div
                style={{
                  fontSize: 10,
        ...
        {Object.entries(grouped).map(([g, items]) => (
          <div key={g} style={{ ...card, padding: 14, marginBottom: 8 }}>
            ...
            {items.map((c) => (
              ...
              >
                <span style={{ color: c.ok ? T.green : T.amber, width: 14 }}>
                  {c.ok ? "✓" : "!"}
                </span>
                <span style={{ minWidth: 180 }}>{c.name}</span>
                <span style={{ color: T.dim, flex: 1 }}>{c.detail}</span>
                {!c.ok && c.fix && (
                  <span style={{ color: T.amber, fontFamily: "monospace", fontSize: 11 }}>
                    {c.fix}
                  </span>
                )}
              </div>
        ...
        <div style={{ ...card, padding: 16 }}>
          {!exp?.overall?.samples ? (
            <div style={{ fontSize: 12.5, color: T.dim }}>
              Nothing observed yet. Once Jarvis has run a few tasks, this shows how reliable each
              app really is on your PC and what actually goes wrong.
            </div>
          ) : (
            <>
              <div
                style={{
                  ...
                  fontWeight: 600,
                  color:
                    exp.overall.rate >= 0.8 ? T.green : exp.overall.rate >= 0.5 ? T.amber : T.red,
                }}
              >
                {Math.round((exp.overall.rate || 0) * 100)}% of the last {exp.overall.samples}{" "}
                actions succeeded
              </div>
                ...
                <div style={{ marginTop: 14 }}>
                  {exp.apps.map((a) => (
                    ...
                    >
                      ...
                      <span
                        style={{ color: a.success_rate >= 0.8 ? T.green : T.amber, minWidth: 46 }}
                      >
      ...
      <div>
        ...
        <div style={{ ...card, padding: 16, display: "flex", gap: 26 }}>
          {[
            ...
            ["worst", data?.traces?.worst_component || "none"],
          ].map(([k, v]) => (
            <div key={k}>
              <div
                style={{
              ...
              <div
                style={{
                  fontSize: 16,
                  fontWeight: 700,
                  color: k === "failed" && v > 0 ? T.red : T.text,
                }}
              >
                {String(v)}
              </div>
              ...
              {open === t.id && (
                <div style={{ marginTop: 10, paddingLeft: 16, borderLeft: `1px solid ${T.line}` }}>
                  {(t.steps || []).map((s, i) => (
                    <div
                      key={i}
                      style={{ display: "flex", gap: 10, fontSize: 12, padding: "4px 0" }}
                    >
                      <span
                        style={{
                          color: s.ok === false ? T.red : s.ok ? T.green : T.dim,
                          width: 12,
                        }}
                      >
                        {s.ok === false ? "✕" : s.ok ? "✓" : "·"}
        ...
        >
          {(data?.capabilities || []).map((c) => (
            ...
            >
              ...
              {!c.available && (
                <span style={{ fontSize: 10, color: T.amber }}>needs {c.missing.join(", ")}</span>
              )}
            </div>
          ))}
        </div>
...
function Metric({ label, value, unit, danger }) {
  return (
    <div style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
      ...
      <span
        className="smooth"
        style={{ fontSize: 13, fontWeight: 600, color: danger ? T.red : T.text }}
      >
